...
Create the sibling project with safe copied source and no secrets.
Refactor local runtime contracts while preserving local tests.
Add Pydantic schemas and the Finance-Only Strict safety validator.
Add DynamoDB, Bedrock, Secrets Manager, and SSM providers.
Add Lambda handlers and async manual-run flow.
Add CDK stacks and deployment scripts.
Add Dockerfile for pipeline Lambda container.
Update dashboard fetch/auth behavior while keeping same-origin /api/_.
Deploy in order:
Core stack
Seed secrets/settings/watchlist/safety policy
Pipeline stack with schedule disabled
Manual pipeline invoke
API stack
Dashboard stack
Enable schedule after successful smoke test
Test Plan
Local:
